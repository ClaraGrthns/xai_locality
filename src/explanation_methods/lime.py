...
class LimeHandler(BaseExplanationMethodHandler):
    ...
    def iterate_over_data(self,
        ...
        for i in range(0, len(tst_feat_for_expl), chunk_size):
            if self.args.debug:
                ...
                for n_closest in n_points_in_ball:
                    ...
                    chunk_results = compute_lime_accuracy_per_fraction(
                        tst_feat_for_expl[i:i+chunk_size], df_feat_for_expl, explanations_chunk, self.explainer, predict_fn, n_closest, tree, predict_threshold
                    )
                ...
                chunk_end = min(i + chunk_size, len(tst_feat_for_expl))
                tst_chunk = tst_feat_for_expl[i:chunk_end]
                explanations_chunk = explanations[i:chunk_end]
