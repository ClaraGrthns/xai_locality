import lime_pkg as lime
import lime_pkg.lime.lime_tabular
...
def accuracy_constraint(dist_threshold, x, dataset, explainer, dist_measure, target_accuracy=0.8):
    ...
    """
    accuracy, _, _ = compute_lime_accuracy(
        x=x,
        dataset=dataset,
        explainer=explainer,
...
def optimize_radius_ball_around_x(x, dataset, explainer, dist_measure="euclidean", target_accuracy=0.85, 
    ...
    final_accuracy, final_ratio, final_radius = compute_lime_accuracy(
        x=x,
        dataset=dataset,
        explainer=explainer,
        predict_fn=predict_fn,
...
explainer = lime_pkg.lime.lime_tabular.LimeTabularExplainer(trn_feat, 
                                                            feature_names=feature_names, 
                                                            class_names=[0,1], 
                                                            discretize_continuous=True)
# Choose a random instances from the test set
idxs = np.random.choice(len(tst_feat), 20)
radiuss = []
final_accuracies = []
ratios = []
for idx in idxs:
...
results_path = "/home/grotehans/xai_locality/results/XGBoost/Jannis/results.npy"
